            ...
            <div class="px-6 py-4">
                ...
                @if($status['is_on_trial'] && $status['has_advanced_during_trial'])
                    <div class="mt-3 grid grid-cols-2 gap-2 md:grid-cols-3">
                        @php
                            $goldPlanFeatures = config('plans.gold.features') ?? [];
                            // Take up to 3 features to display
                            $highlightFeatures = array_slice($goldPlanFeatures, 0, 3);
                        @endphp
                        
                        @foreach($highlightFeatures as $feature)
                        <div class="flex items-center space-x-2">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-{{ $daysLeft <= 3 ? 'amber' : 'blue' }}-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
